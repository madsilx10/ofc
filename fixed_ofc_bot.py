# Onefootball OFC Bot - Fixed Implementation
...
class OnefootballRealBot:
    ...
    def get_wallet_address(self):
        ...
        try:
            ...
            account = self.w3.eth.account.from_key('0x' + self.private_key)
    ...
    def sign_message(self, message):
        """Sign message with private key - Fixed implementation"""
        try:
            account = self.w3.eth.account.from_key('0x' + self.private_key)
            
            # Create the message hash properly
    ...
    def generate_siwe_message(self):
        """Generate properly formatted SIWE message"""
        try:
            # Generate a proper nonce (32 random bytes as hex)
            nonce_bytes = random.randbytes(16)  # 16 bytes = 32 hex chars
            nonce = nonce_bytes.hex()
            
            # Generate proper ISO timestamp
            now = datetime.now(timezone.utc)
            issued_at = now.strftime('%Y-%m-%dT%H:%M:%S.%fZ')[:-3] + 'Z'
            
            # Create properly formatted SIWE message according to EIP-4361
...
- https://privy.io"""
            ...
            return siwe_message, nonce, issued_at
            
        except Exception as e:
            logger.error(f"Failed to generate SIWE message: {e}")
            return None, None, None
    
    def authenticate_wallet(self):
        """Authenticate wallet using SIWE - Fixed implementation"""
        try:
            ...
            siwe_message, nonce, issued_at = self.generate_siwe_message()
            if not siwe_message:
                return False
            
            logger.info("Generated SIWE message:")
            logger.info(siwe_message)
            ...
            logger.info(f"Payload: {json.dumps(auth_payload, indent=2)}")
            ...
            if response.status_code == 200:
                ...
                if 'token' in auth_data:
                    ...
                    self.session.headers['Authorization'] = f"Bearer {self.auth_token}"
                    logger.info("Access token extracted and added to session")
    ...
    def get_campaign_activities(self):
        ...
        try:
            ...
            query = {
                ...
                "query": """query CampaignActivitiesPanel($campaignId: String!, $isTrusted: Boolean!) {
                    campaignActivities(campaignId: $campaignId, isTrusted: $isTrusted) {
                        id
                        createdAt
                        updatedAt
                        startDateTime
